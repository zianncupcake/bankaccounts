import {
  ...
  const WithdrawComponent = ({ getAccount, withdraw }) => {
    const {id} = useParams()
...
const handleSubmit = async (event) => {
    ...
    const newBalance = parseFloat(account.StartingBalance) - parseFloat(form.withdrawAmount.value)
    ...
    const formInputs = {
      ...
      try {
        const res = await withdraw(id, formInputs)
        console.log("res", res)
        setSuccess(true)
      } catch (er) {
        console.log(er)
        ...
        <Row className="justify-content-md-center mt-5">
          ...
          <Form.Group className="mb-3">
              ...
              <Alert variant="danger" className="mt-3" show={showAlert}>
               Withdrawal amount more than account balance
              </Alert>
